function sn1987a_model_lobes_ptsrc,x,y,a,doplot=doplot
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;+
;  This new model needs to be simplified and clearly define the characteristic radius.
;  It should consist of a 2-D Gaussian elliptical (or circular?) torus
;  plus 4 tangentially extended lobes with varying weights + central pointsource. 
;  
;
;  Parameters:  
...
;    xsrc  = location pointsource (tie to x0)
;    ysrc  = location pointsource (tie to y0)  
;    wsrc  = width of the pointsource (in pixels)
;    fsrc  = flux of the source   
;-
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;  g=exp(-0.5*((r-r0)/sigr)^2)
  ...
  ;;Step 2: add lobes
  ;each lobe on torus has position angle,intensity,tangential width (& radial width?)
...
;  t2[w]=t2[w]+2.*!pi
  ...
  rsrc = sqrt((xpos - xsrc)^2d + (ypos - ysrc)^2d)
  imsrc = fsrc[0]*exp(-rsrc^2d/(2d*wsrc^2d))
 

  image2=s0*exp(-(t0)^2/(2.*sigt0^2)) ;*exp(-(r-r0)^2/(2.*sigr0^2imsrc = fsrc[0]*exp(-rsrc^2d/(2d*wsrc^2d)))) ;spot 1
  ...
  image6=image1*[image2+image3+image4+image5]+image1+imsrc ;image1 = ring, multiply to set lobes = 0 in center.
